...
describe("Routes", () => {
  ...
  it('Deve falhar ao criar usuário com senha curta', async () => {
    ...
    expect(response.body.message).toBe('A senha precisa ter no mínimo 6 caracteres');
  });
  ...
  it('Deve pegar os detalhes do usuário', async () => {
    const response = await request(app).get('/usuario/me').set('Authorization', `${authToken}`);
  ...
  it("Deve pegar os detalhes específicos do usuário", async () => {
    const response = await request(app).get('/usuario/mydetails').set('Authorization', `${authToken}`);
  ...
  it('Deve atualizar o usuário com sucesso', async () => {
    const response = await request(app).put('/usuario').set('Authorization', `${authToken}`).send({
      nome: "André",
      sobrenome: "Sales",
      peso: 55
    });
  ...
  it('Deve atualizar o consumo de aguá do usuario com sucesso', async () => {
    const response = await request(app).put('/usuario/agua').set('Authorization', `${authToken}`).send({
      aguaIngerida: 400
    });

    expect(response.status).toBe(200);
    expect(response.body.aguaIngerida).toBe(400)
    expect(response.body.message).toBe('Quantia atualizada! Agora você já consumiu 400ml de agua!');
  })


  it('Deve reiniciar o consumo diário do usuario', async () => {
    const response = await request(app).put('/usuario/agua/zerar').set('Authorization', `${authToken}`).send({});

    expect(response.status).toBe(200);
    expect(response.body.aguaIngerida).toBe(0)
    expect(response.body.message).toBe('Quantia atualizada! Agora você já consumiu 0ml de agua!');
  })

  it('Deve deletar o usuário com sucesso', async () => {
    const response = await request(app).delete('/usuario').set('Authorization', `${authToken}`);
...
describe("Testando rotas de Autenticação", () => {
  it('Deve retornar um erro por não encontrar um token', async () => {
    const loginResponse = await request(app).post('/auth/login').send({
...
describe('Testando as rotas de Alimento', () => {
  it('Deve criar um alimento com sucesso', async () => {
    const response = await request(app).post('/alimento').set('Authorization', `${generalAuthToken}`).send({
  ...
  it('Deve listar todos os alimentos', async () => {
    const response = await request(app).get('/alimento/?page=1&limit=10').set('Authorization', `${generalAuthToken}`);
  ...
  it('Deve buscar um alimento pelo ID', async () => {
    const response = await request(app).get(`/alimento/${alimentoId}`).set('Authorization', `${generalAuthToken}`);
  ...
  it('Deve atualizar o alimento com sucesso', async () => {
    const response = await request(app).put(`/alimento/${alimentoId}`).set('Authorization', `${generalAuthToken}`).send({
  ...
  it('Deve deletar o alimento com sucesso', async () => {
    const response = await request(app).delete(`/alimento/${alimentoId}`).set('Authorization', `${generalAuthToken}`);
  ...
  it('Deve listar alimentos criados pelo usuário', async () => {
    const response = await request(app).get('/alimento/criadosPorMim').set('Authorization', `${generalAuthToken}`);
...
describe('Testando as rotas de Alimento Consumido', () => {
  it('Deve criar um alimento com sucesso', async () => {
    const response = await request(app).post('/alimentoConsumido').set('Authorization', `${generalAuthToken}`).send({
      _id: alimentoId1,
      porcao: 100,
      quantidade: 2,
  ...
  it('Deve remover UMA unidade do alimento com sucesso', async () => {
    const response = await request(app).put('/alimentoConsumido/delete').set('Authorization', `${generalAuthToken}`).send({
      _id: alimentoId1,
      porcao: 100,
      quantidade: 1,
      nomeGrupo: "Café da Tarde"
  ...
  it('Deve listar todos os alimentos', async () => {
    const response = await request(app).get('/alimentoConsumido/me').set('Authorization', `${generalAuthToken}`);
  ...
  it('Deve remover dar erro ao tentar remover mais vezes um alimento que sua quantidade', async () => {
    ...
    await request(app).put('/alimentoConsumido/delete').set('Authorization', `${generalAuthToken}`).send({
      _id: alimentoId1,
      porcao: 100,
      quantidade: 1,
      nomeGrupo: "Café da Tarde"
    ...
    expect(response.body.message).toBe("Alimento consumido não encontrado ou já removido")
  });
...
describe("Testando rotas de Categoria", () => {
  it("Deve listas todas as categorias", async () => {
    const response = await request(app).get('/categoria').set('Authorization', `${generalAuthToken}`);
...
describe("Testando rotas de Dietas Fixas", () => {
  it('Deve criar uma dieta com sucesso', async () => {
    const response = await request(app).post('/dieta').set('Authorization', `${generalAuthToken}`).send({
      diaSemana: ["Sexta-feira", "Segunda-feira", "Quarta-feira", "Quinta-feira"],
      grupos: [
        {
          nome: "Almoço",
          alimentos: [
  ...
  it('Deve atualizar uma dieta pelo seu id', async () => {
    const response = await request(app).put(`/dieta/${dietaId}`).set('Authorization', `${generalAuthToken}`).send({
  ...
  it('Deve listar dietas criadas pelo usuário', async () => {
    const response = await request(app).get('/dieta/me').set('Authorization', `${generalAuthToken}`);
  ...
  it('Deve listar dietas criadas pelo usuário em um dia da semana específico', async () => {
    const response = await request(app).get('/dieta/me?diaSemana=Domingo').set('Authorization', `${generalAuthToken}`);
  ...
  it('Deve buscar uma dieta específica pelo id', async () => {
    const response = await request(app).get(`/dieta/${dietaId}`).set('Authorization', `${generalAuthToken}`);
  ...
  it('Deve deletar uma dieta', async () => {
    const response = await request(app).delete(`/dieta/${dietaId}`).set('Authorization', `${generalAuthToken}`);
  ...
  it('Deve buscar uma dieta específica pelo id e falhar por ter sido apagada', async () => {
    ...
    expect(response.status).toBe(404);
    expect(response.body.message).toBe("Dieta não encontrada ou já removida.")
  });
})
...
describe("Testando rotas de Dietas Diarias", () => {
  ...
  it("Deve criar uma dieta para hoje", async () => {
    await request(app).post('/dieta').set('Authorization', `${dietaAuthId}`).send({
    ...
    const response = await request(app).get('/dietaDiaria/me/forma').set('Authorization', `${dietaAuthId}`)

    expect(response.body[0]).toHaveProperty("diaSemana")
    expect(response.body[0]).toHaveProperty("gruposConsumo")
    expect(response.body[0].gruposConsumo[0].alimentos[0]).toHaveProperty("consumido")
    expect(response.body[0].gruposConsumo[0].alimentos[0]).toHaveProperty("paraConsumir")
    expect(response.body[0].gruposConsumo[0].alimentos[0]).toHaveProperty("alimento")
  ...
  it('Deve listar a dieta diária criada pelo usuário em um dia da semana específico', async () => {
    const response = await request(app).get('/dietaDiaria/me?diaSemana=Domingo').set('Authorization', `${dietaAuthId}`);
  ...
  it('Deve listar todas as dietas diárias criadas pelo usuário', async () => {
    const response = await request(app).get('/dietaDiaria/me').set('Authorization', `${dietaAuthId}`);
  ...
  it('Deve buscar uma dieta diária por ID', async () => {
    const response = await request(app).get(`/dietaDiaria/${dietaDiariaId}`).set('Authorization', `${dietaAuthId}`);
  ...
  it("Deve aumentar em um a quantia de alimentos consumidos", async () => {
    await request(app).post('/alimentoConsumido').set('Authorization', `${dietaAuthId}`).send({
      _id: alimentoId1,
      porcao: 100,
      quantidade: 1,
      nomeGrupo: "Almoço"
...
describe('UsuarioFunc', () => {
  ...
  describe('Testando calculadoraTaxaMetabolismoBasal', () => {
    it('Deve calcular a TMB corretamente para masculino', async () => {
      ...
      const tmb = await usuarioFunc.calculadoraTaxaMetabolismoBasal(peso, altura, idade, sexo);
  ...
  describe('Testando calculadoraCaloriasGastas', () => {
    it('Deve calcular as calorias gastas para sedentário', async () => {
      ...
      const caloriasGastas = await usuarioFunc.calculadoraCaloriasGastas(nivelDeSedentarismo, taxaMetabolismoBasal);
  ...
  describe('Testando calcularConsumoDeCaloriaPorDia', () => {
    it('Deve calcular o consumo de calorias para emagrecimento', async () => {
      ...
      const consumo = await usuarioFunc.calcularConsumoDeCaloriaPorDia(objetivo, gastoDeCaloria);
  ...
  describe('Testando checagemAgua', () => {
    it('Deve reiniciar o consumo de água se for um novo dia', async () => {
      const usuario = { agua: { aguaIngerida: 0, atualizacao: new Date() }, save: jest.fn() };
    ...
    it('Deve não reiniciar o consumo de água se não for um novo dia', async () => {
      const usuario = { agua: { aguaIngerida: 500, atualizacao: new Date() }, save: jest.fn() };
      const atualizacao = new Date();
